...
def education_fees_calculation(state: ClientState):
    ...
    prompt="""You are an {edu_type} education profiling expert based in India. Your role is to collect details from the client about their child's **{edu_type} education plan**. 
                ...
                - The client’s name
                - The child’s name
                - The client’s stated expectation for the child’s {edu_type} education: either **Domestic** or **International**

                Your tasks are strictly limited to the following Two steps:

                1. **Confirm the country of {edu_type} education:**
                - If the client has mentioned **International**, you must ask in which country they expect their child to pursue {edu_type} education.  
                ...
                2. *You must also ask which course does the kid wishes to pursue* 
                    → Again, use the tool `clarify_with_user` for this question.
                ...
                **Final Output:**
                ...
                - Child’s {edu_type} education country (resolved from the conversation)
                - Course which the child want to pursue. 
            """
    ...
    def structured_edu(state: Education):
        ...
        return {'education_preference': final_result.education_preference, 'country_preference': final_result.country_preference, 'course_preference':final_result.course_preference }
    ...
    def college_info_scrapper(state: Education):
        ...
        response=llm_fees_scrapper_.invoke(f"Gather overall fees of the course for top 4 colleges  colleges of {country_preference} country, only for course: {course_preference}")
        avg_fees=llm_avg_fees.invoke(f'Calculate the average of fees for the overall colleges mentioned and present this info according to the expected schema, and do ensure that the amount must not be expressed in any units like crore or lakh just pure number i.e 80000 or 100000.... : INFO {response}')
        print(avg_fees)
    ...
    for education_plan in kids_education: 
        ...
        if education_plan['graduation_destination']:
            ...
            ug_final_state=ug_workflow.invoke({'messages': [f"""Hi I am {client_name} and my child's name is {education_plan['name_of_kid']}.
                                          My preference destination for {education_plan['name_of_kid']}'s {edu_type} is {education_plan['graduation_destination']}.
                                          """]})
            print(ug_final_state)
