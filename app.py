...
#response.content
############################################################################## web scrapper tools and agent #############################################################

tavily_tool = [TavilySearchResults(max_results=1, api_key=TAVILY_API_KEY)] # tavily search tool from langchain community
edu_tavily_llm=llm_azure.bind_tools(tavily_tool)                           # scrapper llm 
tavily_tool_dict={t.name: t for t in tavily_tool}                          # tool in dict format

def college_web_scrapper(user_query: str):                                 # custom webscrapper agent
    tool_call=True
